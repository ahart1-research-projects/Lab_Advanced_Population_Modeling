...
//  Amanda Hart
//
//  Biomass Dynamics: Schaefer production 
//////////////////////////////////////////////
...
PARAMETER_SECTION
  ...
  init_number logit_p;  // Logit of initial biomass
  ...
  sdreport_number BKratio; //report std dev of B in last year to K ratio
...
PROCEDURE_SECTION
  ...
  for (year = fyear+1; year <= lyear+1; year++){ // Loop from first year +1 to last year +1 loop by year each time
    bpen = 0.;
    // Below is the schaefer production model
    ...
    obj_fun += 1000*bpen;
    survival = 1.-cat(year-1,2)/biomass(year); // survival = 1 - exploitation rate, pick second column of cat
    fpen = 0.;
  ...
  BKratio = biomass(lyear+1)/K; // biomass in last year to K ratio
...
//  report model estimates
  ...
  report << "r" << " " << r << endl; // print year, space, biomass
  report << "K" << " " << K << endl; // print year, space, biomass
  report << "p" << " " << p << endl; // print year, space, biomass
  report << "sigma" << " " << sigma << endl; // print year, space, biomass
  report << "q" << " " << q << endl; // print year, space, biomass
